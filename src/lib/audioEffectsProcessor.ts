...
export class AudioEffectsProcessor {
   ...
   */
  async applyPhoneLineEffects(audioBlob: Blob): Promise<Blob> {
    try {
      // Decode audio data
   ...
   */
  private createPhoneNoise(context: OfflineAudioContext): AudioBufferSourceNode {
  ...
  private async audioBufferToBlob(buffer: AudioBuffer): Promise<Blob> {
    ...
    view.setUint32(offset, buffer.sampleRate * numberOfChannels * 2, true); offset += 4; // ByteRate
   ...
   */
  async addStaticCrackle(audioBlob: Blob, intensity: number = 0.05): Promise<Blob> {
...
// Singleton instance
let processorInstance: AudioEffectsProcessor | null = null;

export function getAudioEffectsProcessor(): AudioEffectsProcessor {
 ...
 */
export async function applyPhoneEffects(audioBlob: Blob): Promise<Blob> {
