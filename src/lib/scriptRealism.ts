 ...
 */
export function generateTimeSlot(slotNumber: number): string {
  const now = new Date();
 ...
 */
export function generateDate(daysFromNow: number = 1): string {
  const date = new Date();
 ...
 */
export function generateDay(daysFromNow: number = 1): string {
  const date = new Date();
  date.setDate(date.getDate() + daysFromNow);
 ...
 */
export function getSalespersonName(): string {
 ...
 */
export function formatScriptWithRealism(
  text: string,
  context: ScriptContext,
  options: {
    generateTimes?: boolean;
    generateDates?: boolean;
  } = {}
): string {
  let formatted = text;
 ...
 */
function getIndustryOperation(industry: string): string {
 ...
 */
export function setSalespersonName(name: string): void {
 ...
 */
export function removePlaceholderBrackets(text: string): string {
 ...
 */
export function addPauseIndicators(text: string): string {
  return text
    .replace(/\n\n/g, '\n\n[PAUSE 2 seconds]\n\n')
    .replace(/\?\n/g, '?\n[WAIT for response]\n');
}
