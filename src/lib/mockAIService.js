...
Add any specific details about the conversation, objections raised, or particular interests mentioned by the prospect.`;
    ...
    async generateFollowUpSuggestions(callData) {
        ...
        if (outcome === 'demo-booked') {
            suggestions.push(`1. Prepare ${prospectInfo.industry}-specific demo scenarios and examples`, `2. Send calendar invite with demo agenda and any prep materials`, `3. Research ${prospectInfo.company} online to personalize the demo`);
        }
        else if (outcome === 'follow-up-scheduled') {
            suggestions.push(`1. Send relevant case studies for ${prospectInfo.industry} companies`, `2. Prepare ROI calculator specific to their business size`, `3. Schedule follow-up call with implementation timeline`);
        }
        else if (outcome === 'not-interested') {
            suggestions.push(`1. Send a thank-you email and stay connected on LinkedIn`, `2. Add to quarterly newsletter list for industry updates`, `3. Set reminder to follow up in 6 months when situation may change`);
        }
        else {
            suggestions.push(`1. Send personalized follow-up email within 24 hours`, `2. Include relevant resources based on discussed pain points`, `3. Propose specific next steps (demo, trial, or discovery call)`);
        }
        return suggestions;
    }
};
