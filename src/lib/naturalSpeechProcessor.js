 ...
 */
const getSpeechPatterns = (persona) => {
 ...
 */
function addFillerWords(text, patterns, probability = 0.3) {
 ...
 */
function addHesitations(text, patterns, probability = 0.2) {
    const sentences = text.split(/\.\s+/);
 ...
 */
function addThinkingPauses(text, probability = 0.15) {
    const sentences = text.split(/\.\s+/);
    return sentences.map((sentence, index) => {
 ...
 */
function addInterruptions(text, patterns, probability = 0.1) {
 ...
 */
function addIncompleteThoughts(text, persona, probability = 0.1) {
    // Only for indecisive or highly emotional personas
    if (persona.personality.decisive > 6)
        return text;
    if (Math.random() < probability) {
        const sentences = text.split(/\.\s+/);
 ...
 */
export function addNaturalSpeechPatterns(text, persona, options = {}) {
    const { fillerProbability = 0.25, hesitationProbability = 0.2, pauseProbability = 0.15, interruptionProbability = 0.1, incompleteProbability = 0.08 } = options;
    let processed = text;
 ...
 */
export function getProcessingIntensity(persona) {
    const { emotional, decisive, skeptical, talkative } = persona.personality;
    // Base probabilities
    let filler = 0.25;
    let hesitation = 0.2;
