...
import type { ConversationContext, AIMessage } from './types/aiRolePlayTypes';

export interface ConversationMemory {
  keyPoints: string[];  // Important things discussed
  salespersonPromises: string[];  // Things salesperson committed to
  unresolvedConcerns: string[];  // Prospect's concerns not fully addressed
 ...
 */
export function updateConversationMemory(context: ConversationContext): ConversationMemory {
 ...
 */
function checkIfConcernAddressed(concern: string, messages: AIMessage[], concernMessageId: string): boolean {
  const concernIndex = messages.findIndex(m => m.id === concernMessageId);
  if (concernIndex === -1) return false;
 ...
 */
function hasSignificantMemory(memory: ConversationMemory): boolean {
  return (
    memory.priceDiscussed ||
