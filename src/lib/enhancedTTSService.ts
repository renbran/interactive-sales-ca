...
export type TTSProvider = 'browser' | 'elevenlabs' | 'playht' | 'openai';
...
interface VoiceMapping {
  ...
  playht?: string; // Voice ID
  openai?: 'alloy' | 'echo' | 'fable' | 'onyx' | 'nova' | 'shimmer';
}

// Map personas to voices across providers
const PERSONA_VOICE_MAPPINGS: Record<ProspectPersonaType, VoiceMapping> = {
  'eager-student': {
...
class EnhancedTTSService {
   ...
   */
  configure(config: Partial<TTSConfig>) {
   ...
   */
  onStateChange(callback: (speaking: boolean) => void) {
   ...
   */
  private async speakBrowser(text: string, persona: ProspectPersona): Promise<void> {
   ...
   */
  private async speakElevenLabs(text: string, persona: ProspectPersona): Promise<void> {
   ...
   */
  private async speakOpenAI(text: string, persona: ProspectPersona): Promise<void> {
   ...
   */
  private async speakPlayHT(text: string, persona: ProspectPersona): Promise<void> {
   ...
   */
  isProviderConfigured(provider: TTSProvider): boolean {
    switch (provider) {
      case 'browser':
